...
import net.minecraft.world.item.alchemy.PotionUtils.*
import net.minecraft.world.item.{Item, ItemStack, PotionItem}
...
class BeakerItem(pProperties: Properties) extends PotionItem(pProperties) {
  override def finishUsingItem(pStack: ItemStack, pLevel: Level, pEntityLiving: LivingEntity): ItemStack = {
    val player = pEntityLiving match
      case playerasPlayer: Player => playerasPlayer
      case _ => null
    player match
      case playerasPlayer: ServerPlayer => CriteriaTriggers.CONSUME_ITEM.trigger(playerasPlayer, pStack)
      case _ =>
